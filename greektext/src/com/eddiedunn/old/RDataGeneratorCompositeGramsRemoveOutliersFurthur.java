...
public class RDataGeneratorCompositeGramsRemoveOutliersFurthur {
	...
	private static void runCompositeGramTF_IDFFeature(int chap, Corpus c, SortedMap<String, Integer> tmpGrandCompositeGrams){
		...
		CU.writeCountMapToFile(tmpGrandCompositeGrams, "compositeGramGlobalCountsRemoveOutliersFurthurChap"+String.format("%02d", chap));
		CU.writeVectorToFile(tmp.toArray(new String[0]), "compositeGramFeatureVectorRemoveOutliersFurthurChap"+String.format("%02d", chap));
		CU.writeVectorToFile(c.getManuscriptLabels(chap), "compositeGramManuscriptNameVectorRemoveOutliersFurthurChap"+String.format("%02d", chap));
		
		c.calculateTF_IDF_CompositeGramWeights( tmpGrandCompositeGrams, chap);
		System.out.println("chap "+chap+" finished calculate");
		c.writeCurrentTFIDFFeatureMatrix(tmpGrandCompositeGrams, "CompositeGramIDFFeatureMatrixRemoveOutliersFurthurChap"+String.format("%02d", chap));
		...
		c.writeCurrentCosineMatrix(chap,tmpGrandCompositeGrams, "CompositeGramCosineMatrixRemoveOutliersFurthurChap"+String.format("%02d", chap));
	...
	private static void runCompositeGramTF_IDFFeature(Corpus c, SortedMap<String, Integer> tmpGrandCompositeGrams){
		ArrayList<String> tmp = new ArrayList<String>(tmpGrandCompositeGrams.keySet());
		CU.writeCountMapToFile(tmpGrandCompositeGrams, "compositeGramGlobalCountsRemoveOutliersFurthur");
		CU.writeVectorToFile(tmp.toArray(new String[0]), "compositeGramFeatureVectorRemoveOutliersFurthur");
		CU.writeVectorToFile(c.getManuscriptLabels(), "compositeGramManuscriptNameVectorRemoveOutliersFurthur");
